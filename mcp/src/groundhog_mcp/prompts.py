...
def audit_hidden_text(url: str) -> str:
    ...
    return (
        ...
        "3. Report the difference. For each entry in `threats`, give its `type`, its "
        "`reason` (which signal caught it — `display:none`, near-zero opacity, off-screen, "
        "`content-visibility:hidden`, invisible Unicode, and so on), its `location` in the "
        "DOM, and the text itself.\n\n"
